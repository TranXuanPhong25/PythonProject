...
static constexpr U64 castlingKey[16] = {0,
   ...
   template <PieceType piece, bool promoted>
   Move make(Square source = NO_SQ, Square target = NO_SQ)
   {
      return Move((uint16_t)source | (uint16_t)target << 6 | (uint16_t)piece << 12 | (uint16_t)promoted << 15);
   }
...
#error "Compiler not supported."
   ...
   public:
      ...
      template <Piece p>
      constexpr U64 pieces() const { return piecesBB[p]; }

      template <PieceType p, Color c>
      constexpr U64 pieces() const { return piecesBB[p + c * 6]; }
...
namespace Movegen
{
   ...
   template <Color c, Movetype mt>
   U64 LegalKingMovesCastling(const Board &board, Square sq)
   {
      U64 bb;
    ...
    *******************/
   template <Movetype mt>
   void legalmoves(Board &board, Movelist &movelist, int start_index = 0)
   {
